...
class Game:
    ...
    def load_data(self):
        ...
        self.title_font = path.join(img_dir, 'ZOMBIE.TTF')
        ...
        self.mob_img = pg.image.load(path.join(img_dir, MOB_IMG)).convert_alpha()
        ...
        for type in EFFECTS_SOUNDS:
            self.effects_sounds[type] = pg.mixer.Sound(path.join(snd_dir, EFFECTS_SOUNDS[type]))
        self.weapon_sounds = {}
        ...
        for snd in PLAYER_HIT_SOUNDS:
            self.player_hit_sounds.append(pg.mixer.Sound(path.join(snd_dir, snd)))
    ...
    def update(self):
        while True:
            try:
                m = self.client.rxbuf.get_nowait()
                #print(f"m: {m}")
                peer = m[1]
                match m[0]:
                    case MSG_PLAYER:
                        if m[1] not in self.peers:
                            self.peers[peer] = Peer(self, m[2], m[3], m[4], m[5])
                        else:
        ...
        #print(f"prev: {self.prev_pp}")
        if self.prev_pp == None or (pp[0].distance_squared_to(self.prev_pp[0]) > 25**2) or abs(pp[1] - self.prev_pp[1]) > 25 or pp[2] != self.prev_pp[2]:
            # TODO use MSG_PLAYER
            self.client.txbuf.put(struct.pack(PLAYER_PACKET, b'p', self.pid, self.player.pos.x, self.player.pos.y, self.player.rot, self.player.health))
            self.prev_pp = pp
    ...
    def draw(self):
        ...
        for sprite in self.all_sprites:
            ...
            if self.draw_debug:
                # TODO add hit_rect for item, peer?
                try:
