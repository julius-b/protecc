...
.Requirements
* allow reconnect (retaining player state)
...
.graphics:
* map: https://kenney.nl/assets/top-down-shooter (CC0)
* items: https://kenney.nl/assets/pixel-shmup (CC0), https://kenney.nl/assets/desert-shooter-pack (CC0), https://kenney.nl/assets/space-shooter-redux (CC0)
...
[source,shell]
----
#$ ffmpeg -i mp4 -vn -acodec copy boom-audio.aac
$ ffmpeg -i mp4 -vn boom-audio.wav
$ ffmpeg -ss 4.4 -i boom-audio.wav -t 1 -c copy boom1.wav
$ ffmpeg -ss 6.15 -i boom-audio.wav -t 0.9 -c copy boom2.wav
----
